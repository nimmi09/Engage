  ...
  client.query(
    ...
    (err, results) => {
      ...
      client.query(
        ...
        [user_id_offence,offender_id,loc_id,date_committed,id,victim_id,category_id],
        (err, results1) => {
          if (err) {
            throw err;
          }
        ...
        (err, results) => {
          ...
          client.query(
            `INSERT INTO offender (user_id,offender_age,offender_name,offender_gender,offender_id,date_added,image_id)
                VALUES ($1, $2, $3, $4,$5,$6,$7)`,
            [user_id_offender,offender_age,offender_name,offender_gender,offender_id,date_added,image_id],
            (err, results1) => {
              if (err) {
                throw err;
              }
...
static offence_requests=function(cb){
 console.log('called')
...
static add_offender=function(Offender,cb){
    ...
    client.query(
        ...
        [Offender.user_id,Offender.age,Offender.gender,Offender.date_added,Offender.name,Offender.image_id],
        (err, results) => {
          if (err) {
              console.log(err);
            return cb(err);
...
client.query(
    `INSERT INTO tempusers (type,user_name, email,user_designation, password)
        VALUES ($1, $2, $3, $4,$5)`,
    [User.type,User.user_name, User.email,User.user_designation, User.password],
    (err, results) => {
      if (err) {
        throw err;
      }
